...
### 红包推荐展示格式（mode=redpacket 时使用）

当外卖搜索不可用（美团联盟未配置 + 饿了么未配置），降级为红包推荐模式：品类引导 + 红包转链，用户自行领券下单。

```
🎫 {slotEmoji} 帮你推荐 {slotName} 外卖品类，领红包再下单更划算～

---

🥇 推荐品类：{category1}

> 🧧 [领美团外卖红包]({meituanH5}) | [领饿了么红包]({elemeH5})
...
> 💡 领券后自行在美团/饿了么 App 搜索对应品类即可下单，佣金自动归你～
> 
> 说"换一批"换个品类，或告诉我具体想吃啥帮你精准推荐～
```
